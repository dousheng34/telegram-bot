# -*- coding: utf-8 -*-
import requests, time, os, logging, threading, re
...
VIDEOS = {
    ...
    "ойбай": [
        {
            "quiz_id":      "oybay0",
            "file_id":      "BAACAgIAAxkDAAIGLmm9TNwjOGpTssENWhM5oGtk9kp9AALHmwAC3jrpSbkwfXRVufWKOgQ",
    ...
    # ───── 6. ЯПЫР-АЙ ─────
    "япыр-ай": {
        "file_id":      "BAACAgIAAxkDAAIGMmm9TRHRWnd21fnJDA2D1heVci6DAALUmwAC3jrpSXru0Js4AZL6OgQ",
    ...
    # ───── 7. ӘЙ ─────
    "әй": {
        "file_id":      "BAACAgIAAxkDAAIGM2m9TS5FrkzUc9MrkFbjWuqkev5yAALYmwAC3jrpSWGyAZFbcOlOOgQ",
    ...
    # ───── 8. ӘТТЕГЕН-АЙ ─────
    "әттеген-ай": {
        "file_id":      "BAACAgIAAxkDAAIGNGm9TTovVnODf4SUZAABFwE84gHqhAAC2psAAt466Um6R9NGG2L_HToE",
    ...
    # ───── 9. ҚҰДАЙЫМ-АУ ─────
    "құдайым-ау": {
        "file_id":      "BAACAgIAAxkDAAIGMWm9TP7k9zaHgvyfkqMEwQ_v6oCiAALSmwAC3jrpSbUyJzxi9ddWOgQ",
    ...
    # ───── 10. МӘ ─────
    "мә": {
        "file_id":      "BAACAgIAAxkDAAIGNWm9TUHBhblPZXXaqVUU1M_sEkfEAALcmwAC3jrpSctO59Kfr6bPOgQ",
...
def self_ping():
    ...
    while True:
        time.sleep(300)
        try: session.get(APP_URL, timeout=10)
        except Exception as e: log.warning(f"Self-ping қатесі: {e}")
...
def set_bot_commands():
    ...
    except Exception as e: log.error(f"set_bot_commands: {e}")
...
def send(cid, text, kb=None):
    ...
    try: session.post(f"{BASE}/sendMessage", json=d, timeout=10)
...
def edit(cid, mid, text, kb=None):
    ...
    if kb: d["reply_markup"] = kb
    try: session.post(f"{BASE}/editMessageText", json=d, timeout=10)
...
def on_message(msg):
    ...
    nav = {
        "📋 Тізім":  "/list", "📋 Список": "/list",
        "❓ Қолдау": "/help", "❓ Помощь": "/help",
        "🌐 Тіл":    "/lang", "🌐 Язык":   "/lang",
    }
...
def on_callback(cb):
    try:
        ...
        data = cb.get("data", "")
        try: session.post(f"{BASE}/answerCallbackQuery",
        ...
        if data.startswith("q|"):
            ...
            if vd:
                ...
                if ans == c:
                    ...
                    edit(cid, mid, t(cid, "wrong", user=opts[ans], correct=opts[c]),
                         kb=back_kb(cid))

        elif data == "menu":
            send(cid, t(cid, "welcome"), kb=main_kb(cid))

    except Exception as e: log.error(f"on_callback: {e}")
...
def process(u):
    ...
    except Exception as e: log.error(f"process(): {e}")
